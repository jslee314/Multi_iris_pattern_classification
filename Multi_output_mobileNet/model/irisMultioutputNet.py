...
def efficientNet_output(model_name, load_weights=None, input_tensor=None, classes=1000, output_names = None):
    ...
    return factory[model_name].efficientNet(output_names)
...
class EfficientNet(LayerBase):
    ...
    def efficientNet(self, class_name):
        ...
        x = self.conv_block(image_input,
                            ...
                            kernel=3, strides=2, scope=class_name+'stem_conv1',
        ...
        for idx, (repeat, kernel, exp, out, se, nl, stride, expand_ratio) in enumerate(cfg):
            ...
            x = self.mobilenet_v3_bneck(x, kernel, exp, out, se, nl,
                                        ...
                                        scope=class_name+'block%d_mbconv1' % (idx + 1),
                                        strides=stride,
            ...
            for i in range(repeat - 1):
                x = self.mobilenet_v3_bneck(x, kernel, exp, out, se, nl,
                                            kernel_initializer=CONV_KERNEL_INITIALIZER,
                                            dense_initializer=DENSE_KERNEL_INITIALIZER,
                                            scope=class_name+'block%d_mbconv%d' % ((idx + 1), (i + 2)),
                                            strides=stride,
                                            expand_ratio=expand_ratio,
                                            drop_connect=drop_connect * idx)
        ...
        x = self.conv_block(x,
                            ...
                            kernel_initializer=CONV_KERNEL_INITIALIZER,
                            kernel=1, strides=1, scope=class_name+'head_conv1',
                            nl='swish', use_bias=False)
        x = GlobalAveragePooling2D()(x)
        if self.dropout_rate > 0:
            x = Dropout(self.dropout_rate)(x)
        x = Dense(self.classes, kernel_initializer=DENSE_KERNEL_INITIALIZER)(x)
        print(class_name + str(self.classes))
        x = Activation(activation='softmax', name=class_name)(x)
    ...
    def get_config(self):
        ...
        cfg = [
            ...
            # nl :  활성함수
            # stride: 스트라이드
            # expand_ratio :  channel 의 팽창계수(mobileNetV3)swish
